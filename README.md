...
```
Дополняю:
В v2rayn , удаленный днс ставите https://8.8.8.8/dns-query. или https://1.1.1.1/dns-query,
чтобы не было утечек я ошибся написав tls://8.8.8.8 просто думал баг что на 53 порт шло но нет xray схавал но все ровно слал на 53 и была утечка домена так что советую сделать и такой doh в v2rayn и в браузере doh чтобы не было утечек домена
Далее чтобы все заводилось с холодного нам надо добавить айпи клоудфаер в расширенные настройки днс в поле dns hosts
пишем:
```
nslookup yourdomain.com
```
далее берем 2 записи айпив4 заходим в настройки v2rayn , днс,расширенные и вставляем вот так в нижнее поле:
```
yourdomain.com  ipv4 ipv4
```
Заместо ipv4 ставите айпи клоудфаер.

Зачем это надо? Когда v2rayn стартует на холодном она не знает айпи клоды и тд тп когда у нас указан doh в удаленном днс но v2rayn не знает айпи чтобы отрезолвить ему нужен тунель прокси а построить он без айпи его не может вот тут и тупняк происходит , когда мы добавили под наш домен 2 айпи теперь не будет утечек домена получается они не будут светится проверить можно в wireshark посомотрев какие днс отправляет ваш пк есле нету пакетов с явным указанием вашего домена значит все сделано правильно
